...
impl FileWriter {
    ...
    pub fn start(&mut self) -> Result<(), String> {
        ...
        rotation_handle
            .join()
            .unwrap_or_else(|e| Err(format!("Failed trying to join. Reason: {:?}", e)))?;
        Ok(())
    }

    pub fn start_async(mut self) -> JoinHandle<Result<(), String>> {
        thread::spawn(move || self.start())
    }
    ...
    fn open_file(
        ...
        let mut file = options
            .create(true)
            .open(filepath)
            .unwrap_or_else(|_| panic!("Open the log file {:?}", filepath));
        if with_starting_msg {
            let _ = Self::write_with(file.borrow_mut(), starting_msg.as_bytes());
        }
        Ok(file)
    }
    ...
    fn rotate(&mut self, new_path: PathBuf) -> Result<(), String> {
        fs::rename(self.file_path.clone(), new_path.clone())
            .map_err(|e| {
                format!(
                    "Failed trying to rename the file {:?} to {:?}. Reason: {}",
                    self.file_path.clone(),
                    new_path,
                    e
                )
