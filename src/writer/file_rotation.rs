...
impl FileRotation {
        ...
        loop {
            ...
            if time_for_rotate.gt(&now) {
                ...
                match self.request_rotate() {
                    Err(err) => {
                        error!(
                            "Failed trying to rename the file. Reason: {}",
                            String::from(err)
                        );
    ...
    fn request_rotate(&self) -> Result<PathBuf, RotateError> {
        ...
        self.tx_file_writer
            ...
            .map_err(|e| {
                RotateError::OtherError(format!("Error sending RenameCommand: {:?}", e))
            })?;
    ...
    fn search_files(path: PathBuf) -> Result<Vec<PathBuf>, RotateError> {
        let files_query = path.to_str().ok_or(RotateError::OtherError(format!(
    ...
    fn next_path(&self, files: &Vec<PathBuf>) -> Result<PathBuf, RotateError> {
        ...
        for file in files.iter() {
            let filename_x = file.file_name().and_then(|fname| fname.to_str()).ok_or(
                RotateError::InvalidFile(format!("invalid file: {:?}", file)),
            )?;
