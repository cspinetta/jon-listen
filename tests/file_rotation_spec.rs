...
fn it_rotate_by_duration() {
    ...
    let _rotation_handle: JoinHandle<Result<(), String>> = file_rotation.start_async();

    let received_msg = file_writer_rx.recv_timeout(Duration::from_secs(
        settings.filewriter.rotation.duration.unwrap() + 5,
    ));
    assert!(received_msg.is_ok());
    assert!(matches!(
        received_msg,
        Ok(FileWriterCommand::Rename(_new_filename))
    ));
